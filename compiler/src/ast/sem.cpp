...
void Id::sem() {
    ...
    /* lookup for function */
        ...
        if (tempEntry != nullptr) {
            ...
            /* infer function params - param of function */
            if (tempEntry->params.front()->type->typeValue == TYPE_FUNC
                && expr->getType()->typeValue == TYPE_FUNC) {
                if (dynamic_cast<Function *>(tempEntry->params.front()->type)->params.size() == dynamic_cast<Function *>(expr->getType())->params.size())
                    for (int i = 0; i < (int)dynamic_cast<Function *>(tempEntry->params.front()->type)->params.size(); i++) {
                        if (dynamic_cast<Function *>(tempEntry->params.front()->type)->params.at(i)->typeValue == TYPE_UNKNOWN)
                            destroyAndCreate(dynamic_cast<Function *>(tempEntry->params.front()->type)->params.at(i), dynamic_cast<Function *>(expr->getType())->params.at(i));
                    }
            }
            ...
            if (exprGen != nullptr) {
                ...
                for (; (i < tempEntry->params.size() && tempExprGen != nullptr); i++, tempExprGen = tempExprGen->getNext()) {
                    ...
                    if (tempEntry->params.at(i)->type->typeValue == TYPE_ARRAY
                        ...
                        && tempExprGen->getExpr()->getType()->typeValue == TYPE_ARRAY
                        && tempExprGen->getExpr()->getType()->ofType->typeValue != TYPE_UNKNOWN) {
                        tempEntry->params.at(i)->type->ofType = tempExprGen->getExpr()->getType()->ofType;
                    }
                    if (tempEntry->params.at(i)->type->typeValue == TYPE_REF
                        && tempEntry->params.at(i)->type->ofType->typeValue == TYPE_UNKNOWN
                        && tempExprGen->getExpr()->getType()->typeValue == TYPE_REF) {
                    ...
                    if (tempExprGen->getExpr()->getType()->typeValue == TYPE_FUNC) {
                        ...
                        if (exprEntry->type->typeValue == TYPE_FUNC) {
                            ...
                            // as expr
                            for (int counter = 0; counter < (int)dynamic_cast<Function *>(paramEntry->type)->params.size(); counter++) {
                                if (dynamic_cast<Function *>(paramEntry->type)->params.at(counter)->typeValue == TYPE_UNKNOWN)
                                    destroyAndCreate(dynamic_cast<Function *>(paramEntry->type)->params.at(counter), dynamic_cast<Function *>(tempExprGen->getExpr()->getType())->params.at(counter));
                            }
                        }
                    }
                    ...
                    if (tempExprGen->getExpr()->getType()->typeValue == TYPE_FUNC) {
                        ...
                        while (counter < paramEntry->params.size() && exprEntry->type->typeValue == TYPE_FUNC) {
                        ...
                        if (dynamic_cast<Function *>(paramEntry->type)->outputType->typeValue == TYPE_UNKNOWN)
        ...
        if (exprEntry->type->typeValue != TYPE_UNKNOWN) {
            ...
            if (clause->getPattern()->getType()->typeValue != TYPE_CUSTOM && clause->getPattern()->getType()->typeValue != TYPE_UNKNOWN) {
                ...
                if (exprEntry->type->typeValue == TYPE_CUSTOM) {
                    ...
                    semError = true;
                    if (SHOW_LINE_MACRO) std::cout << "[LINE: " << __LINE__ << "] ";
                    std::cout << redBG << blackFG << "Error" << defBG << defFG << " at: Line "  << tempBarClauseGen->getClause()->getExpr()->YYLTYPE.first_line << ", Characters " << tempBarClauseGen->getClause()->getExpr()->YYLTYPE.first_column << " - " << tempBarClauseGen->getClause()->getExpr()->YYLTYPE.last_column << std::endl;
            ...
            if (getFinalType(exprEntry->type).first->typeValue == TYPE_CUSTOM || getFnFinalType(exprEntry->type).first->typeValue == TYPE_CUSTOM) {}
                ...
                semError = true;
                if (SHOW_LINE_MACRO) std::cout << "[LINE: " << __LINE__ << "] ";
                std::cout << redBG << blackFG << "Error" << defBG << defFG << " at: Line "  << expr->YYLTYPE.first_line << ", Characters " << expr->YYLTYPE.first_column << " - " << expr->YYLTYPE.last_column << std::endl;
...
void For::sem() {
    ...
    if (start->getType()->typeValue != TYPE_INT) {
        ...
        semError = true;
        if (SHOW_LINE_MACRO) std::cout << "[LINE: " << __LINE__ << "] ";
        std::cout << redBG << blackFG << "Error" << defBG << defFG << " at: Line "  << start->YYLTYPE.first_line << ", Characters " << start->YYLTYPE.first_column << " - " << start->YYLTYPE.last_column << std::endl;
    ...
    if (end->getType()->typeValue != TYPE_INT) {
        ...
        semError = true;
        if (SHOW_LINE_MACRO) std::cout << "[LINE: " << __LINE__ << "] ";
        std::cout << redBG << blackFG << "Error" << defBG << defFG << " at: Line "  << end->YYLTYPE.first_line << ", Characters " << end->YYLTYPE.first_column << " - " << end->YYLTYPE.last_column << std::endl;
...
void While::sem() {
    ...
    if (loopCondition->getType()->typeValue != TYPE_BOOL) {
        ...
        if (SHOW_LINE_MACRO) std::cout << "[LINE: " << __LINE__ << "] ";
        std::cout << redBG << blackFG << "Error" << defBG << defFG << " at: Line "  << loopCondition->YYLTYPE.first_line << ", Characters " << loopCondition->YYLTYPE.first_column << " - " << loopCondition->YYLTYPE.last_column << std::endl;
...
void If::sem() {
    ...
    if (condition->getType()->typeValue != TYPE_BOOL) {
        ...
        expectedType.push_back(new Boolean());
        if (SHOW_LINE_MACRO) std::cout << "[LINE: " << __LINE__ << "] ";
        std::cout << redBG << blackFG << "Error" << defBG << defFG << " at: Line "  << condition->YYLTYPE.first_line << ", Characters " << condition->YYLTYPE.first_column << " - " << condition->YYLTYPE.last_column << std::endl;
    ...
    if (expr2 == nullptr) {
        if (expr1->getType()->typeValue == TYPE_UNKNOWN) {
            ...
            semError = true;
            if (SHOW_LINE_MACRO) std::cout << "[LINE: " << __LINE__ << "] ";
            std::cout << redBG << blackFG << "Error" << defBG << defFG << " at: Line "  << expr1->YYLTYPE.first_line << ", Characters " << expr1->YYLTYPE.first_column << " - " << expr1->YYLTYPE.last_column << std::endl;
        ...
        if (currDef->mut) {
            ...
            /* array */
                ...
                if (currDef->expr->getType()->typeValue != TYPE_INT) {
                    semError = true;
                    if (SHOW_LINE_MACRO) std::cout << "[LINE: " << __LINE__ << "] ";
                    std::cout << redBG << blackFG << "Error" << defBG << defFG << " at: Line "  << currDef->expr->YYLTYPE.first_line << ", Characters " << currDef->expr->YYLTYPE.first_column << " - " << currDef->expr->YYLTYPE.last_column << std::endl;
                ...
                if (!dString.first.compare(dSet.first)) {
                    ...
                    if (dSet.second.find(dString.first) != dSet.second.end()) {
                        if (defsSE.at(idx)->type->typeValue != TYPE_FUNC) {
                            semError = true;
                            if (SHOW_LINE_MACRO) std::cout << "[LINE: " << __LINE__ << "] ";
                            std::cout << redBG << blackFG << "Error" << defBG << defFG << " between: Lines " << defs.at(idx)->YYLTYPE.first_line << " - " << defs.at(idx)->expr->YYLTYPE.last_line << std::endl;
...
void BinOp::sem() {
    ...
    if (strcmp(op, ";") && strcmp(op, ":=")) {
        ...
        if (tempExpr1 != nullptr && tempExpr2 != nullptr
            ...
            else if (tempExpr2->type->typeValue == TYPE_FUNC && dynamic_cast<Function *>(tempExpr2->type)->outputType->typeValue == TYPE_UNKNOWN && tempExpr1->type->typeValue == TYPE_UNKNOWN)
                tempExpr1->type = dynamic_cast<Function *>(tempExpr2->type)->outputType;
            else
    ...
    if (!strcmp(op, "+") || !strcmp(op, "-") || !strcmp(op, "*") || !strcmp(op, "/") || !strcmp(op, "mod")) {
        ...
        if (expr1->getType()->typeValue == TYPE_INT && expr2->getType()->typeValue == TYPE_INT && expr1->getType()->ofType == nullptr && expr2->getType()->ofType == nullptr) {}
            ...
            if (expr1->getType()->typeValue != TYPE_INT || expr1->getType()->ofType != nullptr) {
                semError = true;
                if (SHOW_LINE_MACRO) std::cout << "[LINE: " << __LINE__ << "] ";
                std::cout << redBG << blackFG << "Error" << defBG << defFG << " at: Line "  << expr1->YYLTYPE.first_line - 1 << ", Characters " << expr1->YYLTYPE.first_column + 1 << " - " << expr1->YYLTYPE.last_column << std::endl;
            ...
            if (expr2->getType()->typeValue != TYPE_INT || expr2->getType()->ofType != nullptr) {
                semError = true;
                if (SHOW_LINE_MACRO) std::cout << "[LINE: " << __LINE__ << "] ";
                std::cout << redBG << blackFG << "Error" << defBG << defFG << " at: Line "  << expr2->YYLTYPE.first_line << ", Characters " << expr2->YYLTYPE.first_column - 1 << " - " << expr2->YYLTYPE.last_column + 1 << std::endl;
            ...
            if (expr1->getType()->typeValue == TYPE_REF && expr1->getType()->ofType->typeValue == TYPE_UNKNOWN) {
                ...
                && getRefFinalType(expr1->getType()).first == getRefFinalType(expr2->getType()).first) {
                    ...
                    if (expr1->YYLTYPE.first_line == expr2->YYLTYPE.first_line) {
                        semError = true;
                        if (SHOW_LINE_MACRO) std::cout << "[LINE: " << __LINE__ << "] ";
                        std::cout << redBG << blackFG << "Error" << defBG << defFG << " at: Line "  << this->YYLTYPE.first_line << ", Characters " << expr1->YYLTYPE.first_column << " - " << expr2->YYLTYPE.last_column << std::endl;
                    }
                    else {
                        semError = true;
                        if (SHOW_LINE_MACRO) std::cout << "[LINE: " << __LINE__ << "] ";
                        std::cout << redBG << blackFG << "Error" << defBG << defFG << " between: Lines " << expr1->YYLTYPE.first_line << " - " << expr2->YYLTYPE.last_line << std::endl;
                    }
            ...
            if (exprGen != nullptr) {
                ...
                while (tempExprGen != nullptr) {
                    ...
                    if ((long unsigned)i == dynamic_cast<CustomId*>(tempEntry->type)->getParams().size()) {
                        semError = true;
                        if (SHOW_LINE_MACRO) std::cout << "[LINE: " << __LINE__ << "] ";
                        std::cout << redBG << blackFG << "Error" << defBG << defFG << " at: Line "  << this->YYLTYPE.first_line << ", Characters " << tempExprGen->getExpr()->YYLTYPE.first_column << " - " << tempExprGen->getExpr()->YYLTYPE.last_column << std::endl;
                        ...
                        && tempExprGen->getExpr()->getType()->typeValue != dynamic_cast<CustomId*>(tempEntry->type)->getParams().at(i)->typeValue) {
                            semError = true;
                            if (SHOW_LINE_MACRO) std::cout << "[LINE: " << __LINE__ << "] ";
                            std::cout << redBG << blackFG << "Error" << defBG << defFG << " at: Line "  << tempExprGen->getExpr()->YYLTYPE.first_line << ", Characters " << tempExprGen->getExpr()->YYLTYPE.first_column << " - " << tempExprGen->getExpr()->YYLTYPE.last_column << std::endl;
                            Error *err = new TypeMismatch(tempExprGen->getExpr()->getType(), dynamic_cast<CustomId*>(tempEntry->type)->getParams().at(i));
                            err->printError();
                        }
                    }
